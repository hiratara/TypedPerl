...
constrMapper :: (MonadState TypeContext m, MonadError TypeError m) =>
                PerlASTMapper (m (PerlType, Constraint, Substitute))
                              (m (M.Map String PerlType, Constraint, Substitute))
                              (m ([PerlType], Constraint, Substitute))
